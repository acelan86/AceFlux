...
Store.prototype.recalculate = function (changedProps) {
    ...
    for (var i = 0; i < changedProps.length; i++) {
        ...
        for (var d = 0; d < dependents.length; d++) {
            ...
            this.store[dep] = definition.calculate.call(
                this.store,
...
Store.prototype.bindActions = function () {
    ...
    for (var actionName in this.store.actions) {
        if (utils.hasOwn(this.store.actions, actionName)) {
            callbackName = this.store.actions[actionName];
            if (typeof this.store[callbackName] !== 'function') {
                throw 'Callback \'' + callbackName + '\' defined for action \'' + actionName + '\' should be a method defined on the store!';
            }
            /* And `actionName` should be a name generated by `utils.generateActionName` */
            this.listener.on(
                utils.generateActionName(actionName),
                this.store[callbackName].bind(this.store)
            );
        }
    }
};
